...
### D4. Mouse effects: native clicks plus minimal cursor composition

Use ScreenCaptureKit's native `showMouseClicks` indicator and keep single-owner cursor rendering for the whole recording: ScreenCaptureKit owns the system pointer and click indicator from the first frame to the last, while EffectsKit owns the custom dot pointer and highlight. EffectsKit composites only the dot/highlight tile with Core Image over Metal-backed `CVPixelBuffer`s. Its sampler updates cursor position at the configured capture rate; each frame's ScreenCaptureKit metadata supplies the live window position and exact placement inside the output surface. Click-only recordings bypass the compositor, cursor sampler, global event monitors, and app-owned rasterization entirely.

- **Why**: the native click path removes app-owned click monitoring and composition while preserving recorded-only behavior across every capture mode. Overlay windows remain unsuitable for window-isolated capture, so the custom dot and highlight stay in EffectsKit.
- **Existing**: custom effects degrade from full rendering to no highlight and cursor-only rendering when the frame budget is exceeded. Cursor ownership never changes during this degradation, and native click indicators remain owned by ScreenCaptureKit.
- **Planned**: add configurable highlight color/size/opacity, measure actual-capture performance, and reduce capture FPS after the custom effect fallback is exhausted.
...
- **Existing pipeline**: SCK IOSurface frames with native system cursor/click indicators → optional Core Image/Metal dot/highlight composition → incremental `AVAssetWriter` writes. There is no full-frame CPU readback; click-only recordings bypass EffectsKit, while the compositor rasterizes only its small dot/highlight tile on the CPU.
...
- **Existing degradation**: custom effect quality falls back from highlight rendering to cursor-only rendering without changing cursor ownership. Native click indicators remain in ScreenCaptureKit, and the capture delivery path does not yet lower runtime FPS.
