...
Native macOS menu bar app with an existing SwiftUI app target and a local Swift package. The product has three capture modes, driver-free system audio, native cursor and click controls, trim editing, and output controls. Processing stays on-device and requires no account.
...
### D4. Mouse effects: ScreenCaptureKit-owned cursor and clicks

Configure ScreenCaptureKit's `showsCursor` and `showMouseClicks` properties directly. ScreenCaptureKit owns both the system cursor and click indicator from the first frame to the last in every capture mode. Rokuga does not sample cursor state, monitor clicks, or composite app-owned cursor overlays.

- **Why**: native rendering preserves recorded-only behavior across every capture mode and keeps captured IOSurfaces on the direct encoder path without a custom compositor, cursor sampler, or global event monitor.
...
- **Options popover**: segmented controls and toggles for output location, countdown, FPS, audio, native mouse controls, and thumbnail behavior. Advanced encoding lives in Settings.
...
- **Sustained result**: `20260819T160825Z-4k-audio` records 4K60 HEVC with system audio for 10 minutes. Writer drop rate is 0%, output integrity loss is 0.02597%, CPU is 16.67% of one core, steady memory is 47.78 MB, memory slope is 0.00133 MB/s, and measured A/V drift is 11.42 ms/h. Every applicable budget is met.
- **Eight-hour soak**: `20260819T165219Z-4k-audio` finalized 28,800.09 seconds of 4K60 HEVC with system audio into a 47 GiB file. Writer drop rate is 0%, output integrity loss is 0.011789%, CPU is 16.85% of one core, steady/peak memory is 68.23/108.09 MB, memory slope is 0.000986 MB/s, and measured A/V drift is 0.2212 ms/h; the separate endpoint skew is 52.58 ms. Record-to-first-frame is 341.97 ms, finalization is 115.47 ms, and thermal state stayed nominal. Every applicable budget is met.
- **Final representative measurements**: warm-up plus five untraced 30-second runs of motion (`20260820T005937Z-4k-motion`) and system audio (`20260820T010236Z-4k-audio`) have zero writer drops and stay within every budget. Median CPU is 12.72% and 17.33% of one core respectively; median steady memory is 42.80 MB and 47.72 MB.
...
- **Profiler disposition**: current Time Profiler artifacts are `20260819T164702Z-4k-audio-time` and `20260819T164804Z-4k-cfr-h264-time`; the final Allocations artifact is `20260820T011153Z-4k-motion-allocations`. Audio and CFR app-owned self time is at most 0.05%, with AAC, `AVAssetWriter`, and `pwritev` as leaves. No remaining app-owned symbol has 1% self time.
- **File I/O profiling**: Instruments 26.0 (17C52) cannot finalize File Activity or Filesystem Activity even for `/usr/bin/true`. The version-bound `profile file` fallback uses native System Trace without root and exports its syscall table. `20260819T165036Z-4k-audio-file` finalized a 10-second recording, stayed within all capture budgets, and recorded 584 `pwritev`, 1,059 `write`, and 8 `fsync` calls.
...
- **Planned**: require a hardware encoder and report an actionable error when one is unavailable.
...
- **Current validation**: local capture, audio, codec, toolbar, trace, and compare commands produce timestamped artifacts. The audio workload emits host-clock-synchronized visual/audio markers; output A/V drift is the change in marker offset, while track endpoint skew is reported separately. CI does not run performance benchmarks because ScreenCaptureKit measurements require stable local code signing, Screen Recording permission, and controlled hardware.
