...
- Do not commit development work directly to `main`. The automated `chore: release vX.Y.Z` commit created by the **Cut Release** workflow is the exception.
...
- **Cut Release** batches Changie fragments, updates `CHANGELOG.md` and the Xcode project version, commits `chore: release vX.Y.Z` to `main`, and starts the **Release** workflow.
- **Release** uploads the App Store build, creates the GitHub Release and tag from the Changie notes, then updates the Homebrew cask.
